  ...
  return (
    <header
      className={`fixed w-full top-0 left-0 z-50 transition-all duration-300 ${
        headerStyle === 'home' ? 'text-white' : 'text-foreground'
      }`}
    >
            ...
            <div className="hidden sm:block">
              ...
              <p
                className={`text-xs -mt-1 transition-colors duration-200 ${
                  headerStyle === 'home' && !scrolled ? 'text-white/80' : 'text-muted-foreground'
                }`}
              >
            ...
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  {link.name}
                  <span
                    className={`absolute bottom-1.5 left-1/2 w-0 h-0.5 ${
          ...
          <div className="flex flex-col space-y-2 mt-2">
            {navLinks.map((link) => {
              ...
              return (
                ...
                >
                  {link.name}
                  {isActive && <span className="ml-2 w-1.5 h-1.5 rounded-full bg-primary" />}
                </Link>
              );
            })}
          </div>
